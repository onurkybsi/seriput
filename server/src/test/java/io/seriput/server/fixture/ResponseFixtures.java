...
public final class ResponseFixtures {
  ...
  public static Response<?>[] deserialize(
      InputStream inputStream, int expectedLength, Class<?> valueType) throws IOException {
    ...
    while (i < expectedLength) {
      ...
      responses[i] =
          responseDeserializer.deserialize(ByteBuffer.wrap(Bytes.concat(header, body)), valueType);
      i++;
    }
    return responses;
  }
