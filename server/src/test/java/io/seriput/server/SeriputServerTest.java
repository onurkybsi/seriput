...
final class SeriputServerTest {
  ...
  private static final RequestSerializer<String, ObjectNode> serializer =
  ...
  class Lifecycle {
    ...
    @ParameterizedTest
    @EnumSource(
        value = SeriputServer.State.class,
        mode = EnumSource.Mode.EXCLUDE,
        names = {"RUNNING"})
    void should_Throw_IllegalStateException_On_Start_When_Server_Is_Already_Running()
        throws IOException {
      // given
      underTest.start();
    ...
    @Test
    void awaitShutdown_should_Block_Until_Server_Is_Closed()
          ...
          .until(
              () -> {
                if (client.tryToConnect() && !underTest.connections().isEmpty()) {
                  var connections =
                      underTest.connections().values().stream().findFirst().orElseThrow();
          ...
          .until(
              () -> {
                if (client.tryToConnect()) {
                  return serverSelector.keys().stream()
                      .anyMatch(k -> k.attachment() instanceof SeriputConnection);
                }
                return false;
              });
      var selectionKey =
          serverSelector.keys().stream()
              .filter(k -> k.attachment() instanceof SeriputConnection)
              .findFirst()
              .orElseThrow();
  ...
  class ServerShutdown {
    ...
    void should_Close_All_Connections_On_Server_Shutdown() throws Exception {
      ...
      underTest.start();
      var clients =
          IntStream.range(0, 3).mapToObj(_ -> SeriputClient.of("localhost", SERVER_PORT)).toList();
